...
models     = ['LR', 'GBC']
categories = ['traffic', 'time', 'weather', 'geohash', 'NLP']

writer = open('Results/Traditional_For_{}_AllBut.csv'.format(CITY), 'w')
writer.write('Model,AllButCategory,ReportType,F1,Precision,Recall,Support\n')
writer.close()
...
for m in models:
    for c in categories:
        ...
        for i in range(REPEATS):
            df,best_params = make_models(model=m, metric='f1_score', category=cats)        
        ...
        writer = open('Results/Traditional_For_{}_AllBut.csv'.format(CITY), 'a')
